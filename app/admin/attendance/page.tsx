...
const dropdownStyle: React.CSSProperties = {
  ...
  zIndex: 9999,
  background: "#0f1c2e",
  border: "1px solid rgba(46,111,168,0.40)",
  borderRadius: "12px",
  maxHeight: 220,
  overflowY: "auto",
  boxShadow: "0 24px 56px rgba(0,0,0,0.80), 0 0 0 1px rgba(46,111,168,0.15)",
};
...
function Autocomplete({ placeholder, fetchUrl, value, onSelect, mapItems }: {
  ...
  const defaultMap = (item: any) => ({
    id: item.id,
    label: item.fullName || item.name || item.user?.name || 'Sin nombre',
  });
  ...
  const search = async (val: string) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}${fetchUrl}?search=${encodeURIComponent(val)}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await res.json();
      if (!Array.isArray(data)) { setOptions([]); return; }
      const mapper = mapItems ?? defaultMap;
      ...
      {show && (
        <div style={dropdownStyle}>
          ...
          {!loading && options.map(opt => (
            <div key={opt.id}
              onMouseDown={e => { e.preventDefault(); onSelect(opt); setQuery(opt.label); setShow(false); }}
              className="px-3 py-2.5 text-xs cursor-pointer transition-all duration-150"
...
function JustBadge({ just }: { just: { status: string; reason: string } | null }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {just.reason && <span className="text-xs italic max-w-[160px] truncate" style={{ color: "rgba(255,255,255,0.35)" }} title={just.reason}>{just.reason}</span>}
    </div>
  );
}
...
function RegisterModal({ onClose }: { onClose: () => void }) {
  ...
  async function searchByDni() {
    ...
    try {
      const res = await api.get(`/volunteers/by-dni/${dni.trim()}`);
      setVolunteer(res.data);
    } catch {
      setResult({ type: 'err', text: 'No se encontró ningún voluntario con ese DNI' });
  ...
  async function registerDni() {
    ...
    try {
      ...
      setResult({ type: 'err', text: e?.response?.data?.message ?? 'Error al registrar' });
  ...
  async function startCamera() {
    try {
      ...
      setResult({ type: 'err', text: 'No se pudo acceder a la cámara' });
    }
  }
  ...
  // Escanear QR del video
  async function scanQR() {
    if (!sessionId) { setResult({ type: 'err', text: 'Selecciona una sesión primero' }); return; }
    // @ts-ignore
    if (!('BarcodeDetector' in window)) {
      setResult({ type: 'err', text: 'Tu navegador no soporta escaneo QR. Usa Chrome en Android.' });
      stopCamera(); return;
    }
    // @ts-ignore
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        if (codes.length > 0) {
          ...
          setRegistering(true); setResult(null);
          try {
            const res = await api.post('/attendance/scan', { qrToken, sessionId });
            const status = res.data?.status;
            setResult({ type: 'ok', text: `✓ Asistencia registrada correctamente`, status });
          } catch (e: any) {
            setResult({ type: 'err', text: e?.response?.data?.message ?? 'Error al registrar' });
          } finally { setRegistering(false); }
        }
      ...
      <div className="w-full max-w-lg relative overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: `rgba(46,111,168,0.15)` }}>
              <UserCheck size={16} color={BLUE_L} />
            </div>
            <div>
              <p className="font-bold text-sm" style={{ color: "#f1f5f9" }}>Registrar asistencia</p>
              <p className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>Sesión activa</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            {sessions.length === 0 ? (
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs"
                ...
                <AlertCircle size={13} /> No hay sesiones activas en este momento
              </div>
            ) : (
              <select value={sessionId ?? ''} onChange={e => setSessionId(+e.target.value)}
                style={{ ...IS, cursor: "pointer" }} onFocus={fi} onBlur={fo}>
          ...
          <div className="flex gap-2">
            {[
              { key: 'dni', icon: <CreditCard size={13} />, label: 'Por DNI' },
              { key: 'qr',  icon: <QrCode size={13} />,     label: 'Escanear QR' },
            ].map(tab => (
              <button key={tab.key} onClick={() => { setMode(tab.key as any); setResult(null); setVolunteer(null); setDni(''); stopCamera(); }}
                ...
                style={{
                  background: mode === tab.key ? `linear-gradient(135deg,${BLUE},${BLUE_L})` : "rgba(255,255,255,0.05)",
                  color: mode === tab.key ? "#fff" : "rgba(255,255,255,0.45)",
                  border: mode === tab.key ? "none" : "1px solid rgba(255,255,255,0.08)",
                  boxShadow: mode === tab.key ? `0 4px 16px rgba(46,111,168,0.35)` : "none",
                }}>
                {tab.icon} {tab.label}
              </button>
            ))}
          ...
          {mode === 'dni' && (
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" placeholder="Ej: 12345678" maxLength={8}
                    style={IS} value={dni}
                  ...
                  <button onClick={searchByDni} disabled={searching || dni.length < 6}
                    ...
                    style={{
                      background: searching || dni.length < 6 ? "rgba(46,111,168,0.25)" : `linear-gradient(135deg,${BLUE},${BLUE_L})`,
                      color: "#fff", cursor: searching || dni.length < 6 ? "not-allowed" : "pointer",
                      boxShadow: searching || dni.length < 6 ? "none" : "0 4px 12px rgba(46,111,168,0.30)",
                    }}>
                    {searching
              ...
              {/* Datos del voluntario encontrado */}
              {volunteer && (
                <div className="relative overflow-hidden rounded-2xl p-4"
                  style={{ background: "rgba(46,111,168,0.08)", border: "1px solid rgba(46,111,168,0.25)" }}>
                  <div className="flex items-center gap-3">
                    {/* Foto */}
                    <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 flex items-center justify-center"
                      style={{ background: `${BLUE}20`, border: `1px solid ${BLUE}33` }}>
                      {photoUrl
                        ? <img src={photoUrl} className="w-full h-full object-cover" alt="foto" />
                        : <span className="text-xl font-bold" style={{ color: BLUE_L }}>{volunteer.fullName?.charAt(0)}</span>
                      }
                    </div>
                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-sm truncate" style={{ color: "#f1f5f9" }}>{volunteer.fullName}</p>
                      <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.45)" }}>DNI: {volunteer.dni}</p>
                      {volunteer.module?.name && (
                        <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.40)" }}>
                          {volunteer.module.name}
                          {volunteer.sede?.name ? ` · ${volunteer.sede.name}` : ''}
                        </p>
                      )}
            ...
            <div className="space-y-3">
              {!scanning ? (
                <button onClick={startCamera} disabled={!sessionId}
                  className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl text-sm font-semibold transition-all duration-200"
                  style={{
                    background: !sessionId ? "rgba(255,255,255,0.04)" : `rgba(46,111,168,0.12)`,
                    color: !sessionId ? "rgba(255,255,255,0.25)" : BLUE_L,
                    border: `1px dashed ${!sessionId ? "rgba(255,255,255,0.10)" : "rgba(46,111,168,0.40)"}`,
                ...
                <div className="space-y-3">
                  <div className="relative rounded-2xl overflow-hidden"
                    ...
                    <div className="absolute bottom-3 left-0 right-0 flex justify-center">
                      <span className="text-xs px-3 py-1.5 rounded-full font-semibold animate-pulse"
                        style={{ background: "rgba(0,0,0,0.70)", color: GREEN }}>
                        Apunta al código QR del voluntario
                      </span>
                    </div>
                  </div>
          ...
          {result && (
            <div className="flex items-start gap-2 px-4 py-3 rounded-xl text-xs font-semibold"
              ...
              <div>
                <p>{result.text}</p>
                {result.status && (
                  <p className="mt-1 font-bold" style={{ color: result.status === 'puntual' ? GREEN : "#facc15" }}>
                    Estado: {result.status === 'puntual' ? '✓ Puntual' : '⏰ Tarde'}
                  </p>
                )}
              </div>
            </div>
          ...
          {mode === 'dni' && (
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={registerDni} disabled={registering || !volunteer || !sessionId}
                ...
                style={{
                  background: registering || !volunteer || !sessionId
                    ? "rgba(46,111,168,0.25)"
                    : `linear-gradient(135deg,${BLUE},${BLUE_L})`,
                  color: "#fff",
                  cursor: registering || !volunteer || !sessionId ? "not-allowed" : "pointer",
                  boxShadow: registering || !volunteer || !sessionId ? "none" : "0 4px 16px rgba(46,111,168,0.35)",
                }}>
                {registering
...
function VolunteerAttendanceView() {
  ...
  async function loadHistory() {
    ...
    try {
      ...
      const volunteerId = volRes.data.id;
      const res = await api.get(`/attendance/volunteer/${volunteerId}/history`);
      setRows(res.data ?? []);
    } catch { setRows([]); }
    finally { setLoading(false); }
  }
  ...
  async function submitJustification() {
    ...
    try {
      ...
      setMsg({ type: "err", text: e?.response?.data?.message ?? "Error al enviar" });
  ...
  const justBadge = (row: any) => {
    ...
    const cfg =
      row.justification.status === "aprobado"  ? { bg: "rgba(74,222,128,0.14)",  color: "#4ade80",  label: "✓ Aprobado"  } :
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-5 min-h-screen" style={{ background: "#070d14", color: "#e2e8f0" }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Total",     value: rows.length, color: "rgba(255,255,255,0.80)", accent: BLUE      },
        ...
        {loading ? (
          <div className="flex items-center justify-center p-16">
            <div className="text-center space-y-3">
              <div className="w-10 h-10 rounded-full mx-auto animate-spin" style={{ border: `3px solid rgba(46,111,168,0.2)`, borderTopColor: BLUE }} />
              <p className="text-sm" style={{ color: "rgba(255,255,255,0.30)" }}>Cargando asistencias...</p>
            </div>
          </div>
        ) : rows.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-16 space-y-3">
            <ClipboardList size={32} color="rgba(46,111,168,0.25)" />
            <p className="text-sm" style={{ color: "rgba(255,255,255,0.30)" }}>No tienes registros de asistencia aún</p>
          </div>
        ) : (
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea rows={3} placeholder="Explica el motivo de tu falta o tardanza..."
                  style={{ ...IS, resize: "none" }} value={reason}
                  onChange={e => setReason(e.target.value)} onFocus={fi} onBlur={fo} />
              </div>
              <div>
                <label className="text-xs font-semibold uppercase tracking-widest block mb-2" style={{ color: "rgba(255,255,255,0.38)" }}>
                  Archivo adjunto <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 10 }}>(opcional)</span>
                </label>
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button onClick={submitJustification} disabled={saving}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200"
...
interface Row { volunteer: string; module: string; sede: string; session: string; sessionDate: string; status: string; registeredBy: string; createdAt: string; justification: { status: string; reason: string } | null; }
  ...
  const handleExport     = async () => {
    try {
      ...
      const excelData = allRows.map(row => ({
        "Voluntario": row.volunteer, "Módulo": row.module, "Sede": row.sede,
        "Sesión": row.session, "Fecha sesión": row.sessionDate, "Estado": row.status,
        "Justificación": row.justification ? `${row.justification.status} — ${row.justification.reason}` : "—",
        "Registrado por": row.registeredBy || "—",
        "Hora registro": row.createdAt ? new Date(row.createdAt).toLocaleString("es-PE") : "—",
      }));
    ...
    <div className="p-4 md:p-6 space-y-4 md:space-y-5 min-h-screen" style={{ background: "#070d14", color: "#e2e8f0" }}>
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {/* Botón registrar asistencia — solo super_admin y registrador */}
          {canRegister && (
      ...
      <div className="relative p-5" style={{ ...glass(BLUE), overflow: "visible", position: "relative", zIndex: 10 }}>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <Filter size={13} color={BLUE_L} />
            <span className="text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.40)" }}>Filtros</span>
          </div>
          {hasFilters && (
            <button onClick={handleClear} className="flex items-center gap-1 text-xs transition-colors duration-200"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 mb-4">
          <div style={{ position: "relative", zIndex: 44 }}>
            <label className="text-xs uppercase tracking-widest mb-1.5 block" style={{ color: "rgba(255,255,255,0.35)" }}>Voluntario</label>
            <Autocomplete placeholder="Buscar voluntario..." fetchUrl="/volunteers" value={labels.volunteer}
              onSelect={opt => { setFilters(p => ({ ...p, volunteerId: opt?.id })); setLabels(p => ({ ...p, volunteer: opt?.label ?? '' })); }} />
          </div>
          <div style={{ position: "relative", zIndex: 43 }}>
            <label className="text-xs uppercase tracking-widest mb-1.5 block" style={{ color: "rgba(255,255,255,0.35)" }}>Módulo</label>
            <Autocomplete placeholder="Buscar módulo..." fetchUrl="/modules" value={labels.module}
              onSelect={opt => { setFilters(p => ({ ...p, moduleId: opt?.id })); setLabels(p => ({ ...p, module: opt?.label ?? '' })); }} />
          </div>
          <div style={{ position: "relative", zIndex: 42 }}>
            <label className="text-xs uppercase tracking-widest mb-1.5 block" style={{ color: "rgba(255,255,255,0.35)" }}>Sede</label>
            <Autocomplete placeholder="Buscar sede..." fetchUrl="/sedes" value={labels.sede}
              onSelect={opt => { setFilters(p => ({ ...p, sedeId: opt?.id })); setLabels(p => ({ ...p, sede: opt?.label ?? '' })); }} />
          </div>
          <div style={{ position: "relative", zIndex: 41 }}>
            <label className="text-xs uppercase tracking-widest mb-1.5 block" style={{ color: "rgba(255,255,255,0.35)" }}>Sesión</label>
            <Autocomplete placeholder="Buscar sesión..." fetchUrl="/sessions" value={labels.session}
              mapItems={(item: any) => ({ id: item.id, label: item.name || `Sesión ${item.id}` })}
              onSelect={opt => { setFilters(p => ({ ...p, sessionId: opt?.id })); setLabels(p => ({ ...p, session: opt?.label ?? '' })); }} />
          </div>
          <div style={{ position: "relative", zIndex: 1 }}>
            <label className="text-xs uppercase tracking-widest mb-1.5 block" style={{ color: "rgba(255,255,255,0.35)" }}>Desde</label>
            <input type="date" style={IS} value={filters.startDate || ''} onChange={e => setFilters(p => ({ ...p, startDate: e.target.value || undefined }))} onFocus={fi} onBlur={fo} />
          </div>
          <div style={{ position: "relative", zIndex: 1 }}>
            <label className="text-xs uppercase tracking-widest mb-1.5 block" style={{ color: "rgba(255,255,255,0.35)" }}>Hasta</label>
            <input type="date" style={IS} value={filters.endDate || ''} onChange={e => setFilters(p => ({ ...p, endDate: e.target.value || undefined }))} onFocus={fi} onBlur={fo} />
          </div>
        </div>
        <button onClick={handleApply} disabled={loading}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200"
          style={{ background: loading ? "rgba(46,111,168,0.40)" : `linear-gradient(135deg,${BLUE},${BLUE_L})`, color: "#fff", boxShadow: loading ? "none" : "0 4px 16px rgba(46,111,168,0.35)", cursor: loading ? "not-allowed" : "pointer" }}>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            { label: "Total registros", value: stats.total,                 color: "rgba(255,255,255,0.80)", accent: BLUE      },
            { label: "Puntuales",       value: stats.presentes,             color: "#4ade80",                accent: "#4ade80" },
            { label: "Tarde",           value: stats.tarde,                 color: "#facc15",                accent: "#facc15" },
            { label: "Faltas",          value: stats.faltas,                color: "#f87171",                accent: "#f87171" },
            { label: "% Asistencia",    value: `${stats.porcentaje ?? 0}%`, color: BLUE_L,                   accent: BLUE      },
          ].map((card, i) => (
        ...
        <div className="relative overflow-hidden" style={glass(BLUE)}>
          ...
          <div className="px-5 py-4 flex items-center justify-between flex-wrap gap-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <span className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>
              {pagination ? `${pagination.total} resultado(s) · página ${pagination.page} de ${pagination.totalPages}` : `${rows.length} resultado(s)`}
            </span>
          </div>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-16 space-y-3"><ClipboardList size={32} color="rgba(46,111,168,0.25)" /><p className="text-sm" style={{ color: "rgba(255,255,255,0.30)" }}>No se encontraron registros con los filtros aplicados</p></div>
          ) : (
            <>
              <div className="overflow-x-auto">
                ...
                <div className="px-5 py-4 flex items-center justify-between flex-wrap gap-3" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  <div className="flex items-center gap-1">
                    {Array.from({ length: pagination.totalPages }, (_, i) => i + 1)
                      ...
                      .reduce<(number | string)[]>((acc, p, idx, arr) => { if (idx > 0 && (p as number) - (arr[idx - 1] as number) > 1) acc.push('...'); acc.push(p); return acc; }, [])
                      .map((p, i) => p === '...'
                        ? <span key={`e-${i}`} className="px-2 text-xs" style={{ color: "rgba(255,255,255,0.30)" }}>...</span>
                        : <button key={p} onClick={() => handlePageChange(p as number)} disabled={loading} className="w-8 h-8 rounded-lg text-xs font-semibold transition-all duration-200"
                            style={{ background: currentPage === p ? `linear-gradient(135deg,${BLUE},${BLUE_L})` : "rgba(255,255,255,0.05)", color: currentPage === p ? "#fff" : "rgba(255,255,255,0.55)" }}>{p}</button>
                      )}
                  </div>
                  <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === pagination.totalPages || loading}
                    className="flex items-center gap-1.5 text-sm px-3 py-2 rounded-xl transition-all duration-200"
                    style={{ background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.55)", border: "1px solid rgba(255,255,255,0.09)" }}>
      ...
      {!applied && !loading && (
        <div className="relative overflow-hidden flex flex-col items-center justify-center p-16 space-y-4" style={glass()}>
          ...
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: "rgba(46,111,168,0.10)", border: "1px solid rgba(46,111,168,0.18)" }}>
            <BarChart2 size={30} color={BLUE_L} />
          </div>
