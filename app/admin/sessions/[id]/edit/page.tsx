...
export default function EditSessionPage() {
    ...
    Promise.all([api.get(`/sessions/${id}`), api.get("/modules")])
      .then(([sessionRes, modulesRes]) => {
        ...
        setForm({
          ...
          toleranceTime: s.toleranceTime ?? "", endTime: s.endTime ?? "",
          moduleId: mods.length === 1 ? String(mods[0].id) : "",
  ...
  const toggleModule = (modId: number) =>
    setForm(prev => ({
      ...prev,
      moduleIds: prev.moduleIds.includes(modId) ? prev.moduleIds.filter(m => m !== modId) : [...prev.moduleIds, modId],
    }));
    ...
    <div className="p-4 md:p-6 min-h-screen" style={{ background: "#070d14", color: "#e2e8f0" }}>
      <div className="max-w-2xl space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold" style={{ color: "#f1f5f9", letterSpacing: "-0.3px" }}>Editar Sesión</h1>
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>{sessionName}</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden p-6 space-y-5"
          style={{
            ...
            border: `1px solid ${ORANGE}33`, borderRadius: "20px", boxShadow: "0 8px 32px rgba(0,0,0,0.30)",
          }}>
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg,${ORANGE},#f5a35a,transparent)` }} />

          {/* Nombre */}
          <div>
            <Label icon={<CalendarDays size={11} />} required>Nombre</Label>
            <input style={IS} value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} onFocus={fi} onBlur={fo} required />
          </div>

          {/* Descripción */}
          <div>
            <Label icon={<AlignLeft size={11} />} sub="Opcional">Descripción</Label>
            <textarea rows={2} style={{ ...IS, resize: "none" }} value={form.description}
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <input type="time" required
                    ...
                    onChange={e => setForm({ ...form, [key]: e.target.value })} onFocus={fi} onBlur={fo} />
