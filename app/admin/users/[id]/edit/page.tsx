...
export default function EditUserPage() {
  ...
  useEffect(() => {
    usersApi.getById(Number(id)).then(res => {
      setName(res.data.name ?? "");
      setEmail(res.data.email ?? "");
      const role = ROLES.find(r => r.name === (res.data.role?.name ?? res.data.role));
    ...
    <div className="p-4 md:p-6 min-h-screen" style={{ background: "#070d14", color: "#e2e8f0" }}>
      <div className="max-w-lg space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold" style={{ color: "#f1f5f9", letterSpacing: "-0.3px" }}>Editar usuario</h1>
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>{name}</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden p-6 space-y-5"
          ...
          <div>
            ...
            <input style={IS} value={name} onChange={e => setName(e.target.value)} onFocus={fi} onBlur={fo} />
          </div>

          {/* EMAIL */}
          <div>
            <Label icon={<Mail size={11} />} required>Email</Label>
            <input style={IS} value={email} onChange={e => setEmail(e.target.value)} onFocus={fi} onBlur={fo} />
          </div>

          {/* CONTRASEÑA */}
          <div>
            <Label icon={<Lock size={11} />} sub="Dejar vacío para no cambiar">Nueva contraseña</Label>
            <input type="password" style={IS} placeholder="••••••••"
